...
forbid_fragment "$SKILLS_ROOT/advance-issue-frontier/SKILL.md" "gh-verified-push.sh"
forbid_fragment "$PARALLEL_REFERENCE" "gh-verified-push.sh"
forbid_fragment "AGENTS.md" "gh-verified-push.sh"

if grep -Ein 'Rust source|cargo (fmt|test|clippy|build)|Cargo\.toml|crates/|VISION\.md' \
  "$SKILLS_ROOT"/*/SKILL.md >/dev/null; then
  grep -Ein 'Rust source|cargo (fmt|test|clippy|build)|Cargo\.toml|crates/|VISION\.md' \
    "$SKILLS_ROOT"/*/SKILL.md >&2 || true
  fail 'Rust repository instructions leaked into the rusui suite'
fi
