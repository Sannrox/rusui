...
| Process | Sumika for the live PTY child | Rusui records its name and observed lifecycle events |
...
| Running, idle, or blocked | Reports a hook observation | Stores the status and observation time as an event |
...
| Reconnect | Attaches to the same named Process | No new Session or Turn |
| Steal | New Attach disconnects the previous Attach | Process and Session continue |
| Process death | Reaps the child and reports dead | Records process_exited; the durable Session remains inspectable |
| Daemon restart | Rebuilds an empty in-memory Process table | Marks the runtime observation unknown; it does not infer death or restart a possibly orphaned child |
| Explicit restart | Starts the named Process only after the prior instance is dead or resolved | Records a new Process start against the same durable Session |
...
- The Process name is rusui- followed by the Rusui Session's numeric ID. Rusui
  Session IDs are stable SQLite auto-increment identities. This name fits
...
- Experimental: the local session kind, same-user Sumika start/list/kill
  integration, status events, and orphan reporting. This ADR accepts the
  boundary only; no local adapter is implemented by this change.
...
Issues #183–#186 own that implementation and evidence; #141 controls stable
core promotion. Reverse by superseding this ADR before any core promotion;
this decision alone adds no runtime schema.
