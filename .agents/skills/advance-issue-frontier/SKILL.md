...
Read `AGENTS.md`, `README.md`, `ARCHITECTURE.md`, repository instructions, and
live issues and pull requests. Do not rely on a stale local backlog export.
...
surface: `policy.yaml`, admit/lease, GitHub-client, apply, Slack, or
persistence. Read each candidate's affected boundary and scope; when two
candidates name the same surface, present them as sequential, not parallel,
and report the collision for maintainer judgment.
