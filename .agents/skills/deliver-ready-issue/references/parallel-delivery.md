...
3. Reject candidate pairs that touch the same collision surface: `policy.yaml`,
   admit/lease, GitHub-client, apply, Slack, or persistence. Run such issues
   in sequence.
...
1. Each lane publishes to its claim branch from its own worktree with
   ...
   the user asked otherwise, under Publish authority, after
   `git fetch --prune origin` and
