...
defaults:
  ...
  implement: false
  land: false
  max_reviews_per_repo_per_utc_day: 1
projects:
  test:
    repos:
      example/test-repo:
...
func TestReviewCLIBlackboxCreatesAndPrintsDryRunResult(t *testing.T) {
	...
	got := string(output)
	for _, want := range []string{"Review session", `"revision_id": 1`, `"dry_run_actions"`, "dry-run comment operator_review"} {
	...
	if fake.CallCount() != 1 {
		t.Fatalf("initial GitHub reads %d", fake.CallCount())
	}
	...
	if repeated.SessionID != sessionID || repeated.PendingRevision != 1 || repeated.TurnID != turns[0].ID {
		t.Fatalf("repeat returned a new session or revision: %+v", repeated)
	}
	if count, err := store.CountReviews(eng.Store, turns[0].ID); err != nil || count != 1 {
